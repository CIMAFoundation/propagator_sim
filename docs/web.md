...
1. Click the map to place the simulation area's center, then click again to
   ...
   the first click).
2. Select the numba core or the optional native Rust core, then adjust radius,
   cell size, wind, moisture, duration, output frequency, and number of
   realizations. Enable spotting if the scenario should include wind-carried
   embers. The Rust option requires the locally built `propagator_rust` wheel.
