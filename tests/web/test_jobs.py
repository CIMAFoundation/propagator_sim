...
def blocking_runner_factory(
    release_event: threading.Event, started_event: threading.Event
):
...
def test_submit_and_get_returns_expected_state():
    ...
    while (
        job.status not in (JobStatus.DONE, JobStatus.FAILED)
...
def test_cancel_sets_flag_on_job_state():
    ...
    deadline = time.time() + 5
    while (
        manager.get(job_id).status != JobStatus.DONE and time.time() < deadline
    ):
        time.sleep(0.05)
...
def test_submit_evicts_finished_jobs_from_previous_runs():
    ...
    deadline = time.time() + 5
    while (
        manager.get(first_id).status != JobStatus.DONE
        and time.time() < deadline
    ):
        time.sleep(0.05)
