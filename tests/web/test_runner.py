...
@pytest.mark.parametrize("core", ["numba", "rust"])
def test_run_loop_produces_frames_with_consistent_shape_and_growing_area(core):
    if core == "rust":
        pytest.importorskip("propagator_rust")
    request = make_request(core=core)
...
@pytest.mark.parametrize("core", ["numba", "rust"])
def test_schedule_actions_heavy_action_blocks_ignition_cell(core):
    if core == "rust":
        pytest.importorskip("propagator_rust")
    size = 30
    utm_epsg = "EPSG:32633"
