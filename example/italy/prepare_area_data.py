...
def main() -> None:
    ...
    ignition_lat = (
        args.ignition_lat if args.ignition_lat is not None else args.lat
    )
    ignition_lon = (
        args.ignition_lon if args.ignition_lon is not None else args.lon
    )

    print(
        f"Preparing data for ({args.lat}, {args.lon}), radius {args.radius_km} km ..."
    )
    try:
