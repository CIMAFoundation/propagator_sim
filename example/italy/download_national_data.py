...
def main() -> None:
    ...
    if cache_dir is None:
        ...
        cache_dir = Path(env_dir) if env_dir else Path.home() / ".propagator" / "cache"

    print(f"Downloading DEM + land-cover tiles for all of Italy into {cache_dir}")
    ...
    def on_progress(kind: str, name: str, size: int) -> None:
        ...
        if kind in ("dem_ok", "wc_ok"):
            ...
            print(f"  ok   {name} ({size / 1e6:.1f} MB, {downloaded_bytes / 1e9:.2f} GB so far)")
        else:
            print(f"  skip {name} (no coverage)")

    summary = download_italy_tiles(cache_dir=cache_dir, progress_cb=on_progress)

    print("\nDone.")
    print(f"  DEM tiles:        {summary['n_dem_downloaded']} downloaded, {summary['n_dem_missing']} without coverage")
    print(f"  WorldCover tiles: {summary['n_worldcover_downloaded']} downloaded, {summary['n_worldcover_missing']} without coverage")
